#!/usr/bin/env python3
"""
figma-perfect: extract.py
All-in-one Figma data fetcher.
...
Outputs:
    {output}/design.json        — raw API response (full node tree)
    {output}/tokens.json        — extracted design tokens
    {output}/spec.md            — human-readable implementation spec
    {output}/screenshots/       — section screenshots (PNG)
...
def extract_node_tokens(node, font_map):
    ...
    if layout_mode and layout_mode != 'NONE':
        ...
        # Collect padding even without auto layout
        padding = {}
        for key in ('paddingTop', 'paddingRight', 'paddingBottom', 'paddingLeft'):
            val = node.get(key)
            if val is not None and val != 0:
                padding[key.replace('padding', '').lower()] = val
        if padding:
            tokens['padding'] = padding
...
def extract_all_tokens(root_node, font_map):
    ...
    def visit(node, depth):
        ...
        if text and text.get('fontFamily'):
            ...
            # De-duplicate by family+size+weight
    ...
    # Build top-level sections summary
    sections = []
...
def generate_spec(design_name, tokens_data, output_dir):
    ...
    lines = [
        ...
        "",
        "Generated by figma-perfect/scripts/extract.py",
        "",
        "---",
        "",
        "## Design Tokens",
    ...
    lines += [
        ...
        "- Run `diff.py` after implementation to validate accuracy",
        "",
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description='Figma data extractor — fetches everything from a Figma URL in one command'
    )
